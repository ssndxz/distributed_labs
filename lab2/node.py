#!/usr/bin/env python3
"""
Lab 2 Solution Node (3-node ready)
...
def replicate_to_peers(key: str, value: Any, ts: int, origin: str, retries: int = 3, timeout_s: float = 2.0) -> None:
    """
    Send update to all peers via POST /replicate.
    Includes artificial delay for Scenario A and exponential backoff.
    """
    ...
    for peer in PEERS:
        ...
        if delay_s > 0:
            print(f"[{NODE_ID}] Sleeping {delay_s}s before replicating to {peer}...")
            time.sleep(delay_s)
...
def main():
    ...
    if NODE_ID == "A":
        for peer in PEERS:
            if "8002" in peer:
                DELAY_RULES[("A", peer)] = 5.0
                print(f"[{NODE_ID}] Configured 5s delay to {peer} (Scenario A)")
